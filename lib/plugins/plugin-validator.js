...
async function validatePlugin (executionPath, argv) {
      ...
      if (mainConfig && mainConfig.path) {
        ...
        if (pluginConfig) {
          if (isConfigEmpty) {
            ...
            errors.push(`There are no contents in your ${configFileName} file${caveat}!`)
          } else {
  ...
  if (!errors.length > 0) {
    console.log()
    console.log(colors.green(`The plugin config in ${configFileName} is valid.`))
    console.log()
  } else {
    process.exitCode = 100
    console.error()
    errors.forEach(err => console.error(colors.red(`Error: ${err}`)))
    console.error()
  }
}
