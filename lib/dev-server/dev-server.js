...
const { setupWatch } = require('./watch')

const {
  generateAssetsSync,
  generateCssSync,
  proxyUserSassIfItExists, generateManagePrototypeCss, generateNunjucksSync
} = require('../build')
...
function startKit () {
  return new Promise((resolve) => {
    ...
    events.once(eventTypes.KIT_STOPPED, () => {
      resolve(kitFork)
    })
  })
}
...
async function runDevServer () {
  ...
  await generateManagePrototypeCss().catch(console.error)
  ...
  events.on(eventTypes.PLUGIN_LIST_UPDATED, () => {
    console.log('kicking off rebuild and restart.')
