...
const { runSequentially, monitorEventLoop, findAvailablePortWithUserPromiseEdition } = require('../utils')
...
const commands = {
  ...
  'full-rs': () => {
    console.log('received full restart command, this is the command that mimics restarting when a newer/older version of nowprototypeit is installed.')
...
async function startJustUpload () {
  fork(`${path.join(__dirname, 'manage-prototype', 'just-upload-runner.js')}`, {
    env: {
      PORT: await findAvailablePortWithUserPromiseEdition(5617),
      ALREADY_LOGGED_KIT_STARTED: kitStartLogged ? 'true' : 'false'
    },
    passThroughEnv: true,
    neverRejectFinishedPromise: true
...
async function runDevServer () {
  ...
  events.on(eventTypes.MANAGEMENT_RESTART, runSequentially(async () => {
    ...
    const eventType = eventTypes.KIT_STARTED
    const listenForMessagesWhileRestarting = (message) => {
      missedMessages.push(message)
    }
    events.on(eventType, listenForMessagesWhileRestarting)
    await managementFork.close()
    managementFork = await startManagement(port)
    events.removeListener(eventType, listenForMessagesWhileRestarting)
    for (const message of missedMessages) {
      managementFork.emit(eventType, message)
    }
  ...
  events.on(eventTypes.TRIGGER_KIT_RESTART, runSequentially(async function () {
    await kitForkPromise
    kitFork.close()
    kitForkPromise = startKit().catch((e) => {
      verboseLog('kit start failed', e)
    })
