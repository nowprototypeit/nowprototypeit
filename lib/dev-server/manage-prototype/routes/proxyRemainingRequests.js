    ...
    app.use(async (req, res) => {
      ...
      async function sendResponse ({
        ...
        if (userConfig.getConfig().autoReloadPages && headers['content-type']?.startsWith('text/html')) {
          ...
          const editorAfterReplace = editInBrowserHtml.replaceAll('__view_file_path__', JSON.parse(headers['x-npi-edit-page-info'] || '{}').mainViewFile?.replaceAll('\\', '/') || '__no_know_file__')
