...
module.exports = {
  proxyRemainingRequests: (app, config) => {
    ...
    app.use(async (req, res) => {
      ...
      const editInBrowserHtml = await getEditInBrowserMarkup()
      ...
      async function sendResponse ({
        ...
        if (userConfig.getConfig().autoReloadPages && headers['content-type']?.startsWith('text/html')) {
          ...
          const editorAfterReplace = editInBrowserHtml.replaceAll('__view_file_path__', JSON.parse(headers['x-npi-edit-page-info'] || '{}').mainViewFile || '__no_know_file__')
