        ...
        if (statusCode === 404 && headers['content-type'].startsWith('application/json') && getJsonFromResponse().errorToBeDisplayedNicely === true && getJsonFromResponse().is404) {
          if (req.originalUrl.split('?')[0] === '/') {
            return res.render('home', {
              defaultViewExtension: config.useNjkExtensions ? 'njk' : 'html',
              foundPages: recursiveDirectoryContentsSync(path.join(appDir, 'views'))
                .filter(file => file.endsWith('.njk') || file.endsWith('.html'))
                .map(file => ({
                  name: file,
                  url: file.replace(/\.(?:njk|html)$/, '').replaceAll(path.sep, '/')
                }))
                .filter(page => !page.url.startsWith('layouts/') && !page.url.startsWith('includes/'))
                .map(obj => ({
                  ...obj,
                  name: '/' + obj.name.split('.').slice(0, -1).join('.').replaceAll(path.sep, '/')
                }))
            })
          }
