...
const { getMessages } = require('../../../../utils/messages-from-api')

let messagesPromise

function updateMessages () {
  messagesPromise = getMessages(kitVersion, 'management-pages')
}
...
module.exports = {
  setupBasicPages: (router, config) => {
    router.get('/', async (req, res) => {
      ...
      const releaseVersion = kitVersion
      res.render('index', {
        ...getModel('Home', req),
        releaseVersion,
        messages: messages?.messages || [],
        docsUrl: messages?.docsUrl,
