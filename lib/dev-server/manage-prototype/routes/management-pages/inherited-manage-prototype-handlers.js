...
function getPluginTemplates () {
  ...
  plugins.getByType('templates')
    .filter(({ item }) => item.type === 'nunjucks' || (getConfig().respectFileExtensions && (item.type === 'markdown' || item.type === 'html')))
...
async function prepareForPluginPage (isInstalledPage, search) {
  const [allPlugins, installedPlugins] = await Promise.all([getPluginsForDiscovery(), getInstalledPackages()])
...
async function getPluginDetailsHandler (req, res, next) {
  ...
  if (installedVersion?.internalRef && (latestVersion?.internalRef !== installedVersion.internalRef && latestVersion?.internalRef !== installedVersion.internalRef)) {
