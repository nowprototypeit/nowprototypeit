...
function getPluginTemplates () {
  const output = []
  plugins.getByType('templates')
    .filter(({ item }) => item.type === 'nunjucks' || (getConfig().respectFileExtensions && (item.type === 'markdown' || item.type === 'html' || item.type === 'php')))
    .forEach(({ packageName, item }) => {
      const matchingPackages = output.filter(x => x.packageName === packageName)
      if (item.type !== 'nunjucks' && item.type !== 'markdown') {
...
async function getTemplatesHandler (req, res) {
  ...
  if (req.query.prefillUrl) {
    const prefillUrlQsPart = `&prefillUrl=${encodeURIComponent(req.query.prefillUrl)}`
    availableTemplates.forEach(packageDescription => {
      packageDescription.templates.forEach(template => {
        template.installLink += prefillUrlQsPart
      })
    })
  }
...
function getTemplatesInstallHandler (req, res) {
  ...
  if (templateConfig?.templateInvalid) {
    res.status(409).send(templateConfig.templateInvalid)
    ...
    res.render('templates/install.njk', {
      ...
      chosenUrl: req.query['chosen-url'] || req.query.prefillUrl
    })
  } else {
...
async function postTemplatesInstallHandler (req, res) {
  ...
  const installLocation = path.join(appViewsDir, `${chosenUrl}.${templateDetails.fileExtension}`)
  ...
  res.redirect(`/manage-prototype/templates/post-install?chosen-url=${encodeURIComponent(chosenUrl)}&file-path=${encodeURIComponent(chosenUrl + '.' + templateDetails.fileExtension)}`)
}
...
function getTemplatesPostInstallHandler (req, res) {
  ...
  res.render('templates/post-install.njk', {
    ...
    pageName,
    headerSubNavItems: getPageNavLinks(req),
    url: req.query['chosen-url'],
    filePath: path.join('app', 'views', req.query['file-path'])
  })
}
...
async function prepareForPluginPage (isInstalledPage, search) {
  const [allPlugins, installedPlugins] = await Promise.all([getPluginsForDiscovery(), getInstalledPlugins()])

  const plugins = (isInstalledPage ? installedPlugins : allPlugins).filter(({ error }) => !error)
  ...
  const pluginList = await Promise.all(filteredPlugins.map(buildPluginData))
  ...
  return {
    ...
    totalBeforeSearch: plugins.length
  }
}
...
async function getPluginsHandler (req, res) {
  ...
  const { plugins, status, updates = 0, found = 0, totalBeforeSearch = 0 } = await prepareForPluginPage(isInstalledPage, search)
  ...
  const model = {
    ...
    headerSubNavItems: getPageNavLinks(req),
    isInstalledPage,
    isSearchPage: totalBeforeSearch > 7,
    sideNavLinks,
    search,
    plugins,
    updatesMessage,
...
async function getPluginDetailsHandler (req, res, next) {
  ...
  function getInThisPluginDetails () {
    ...
    if (plugin.pluginConfig.markdownRenderers && plugin.pluginConfig.markdownRenderers.length > 0) {
      list.push({
        title: 'Markdown renderers',
        items: plugin.pluginConfig.markdownRenderers.map(x => x.name)
      })
    }
    return list
  }
  ...
  if (installedVersion?.internalRef && (latestVersion?.internalRef !== installedVersion.internalRef && latestVersion?.internalRef !== plugin?.internalRef)) {
  ...
  if (installedVersion?.internalRef !== plugin.internalRef) {
    model.installLink = plugin.links.install
    model.installLinkText = installedVersion?.internalRef ? 'Install this version' : 'Install'
  }
...
async function getRelatedPluginsForUninstall (chosenPlugin) {
  const result = []
  const chosenPackageName = chosenPlugin.packageName
  const installed = await getInstalledPackages()

  function getFullRecursiveList (packageNameToLookFor) {
    installed.filter(x => {
      return (x.pluginConfig?.pluginDependencies || []).some(y => (y.packageName || y) === packageNameToLookFor)
    }).forEach(x => {
      if (result.some(y => y.internalRef === x.internalRef)) {
        return
      }
      result.push(x)
      getFullRecursiveList(x.packageName)
    })

    return result
  }

  getFullRecursiveList(chosenPackageName)
  return result.filter(x => x.packageName !== chosenPackageName)
}
