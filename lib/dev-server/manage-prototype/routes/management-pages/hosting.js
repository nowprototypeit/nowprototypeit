...
async function createTgz (dir) {
  ...
  return tar.c({
    ...
    cwd: dir,
    prefix: 'project/'
  }, (await fsp.readdir(dir)).filter(f => include.includes(f)))
}
...
function hostingEnabled (req, res, next) {
  if (getConfig().hostingEnabled) {
    ...
    res.status(403).send({
      error: 'Hosting is not enabled'
    })
  }
}

function getNavItemsForStandaloneOrEmbedded (req, options) {
  return options?.mode === 'just-upload' ? [] : getPageNavLinks(req)
}

module.exports = {
  setupHostingPages: (router, options) => {
    router.get('/hosting/spinner.css', [hostingEnabled], (req, res) => {
      res.sendFile(path.resolve(__dirname, '..', '..', 'assets', 'css', 'spinner.css'))
    })
    router.get(['/hosting', '/npi-cloud-upload'], [hostingEnabled], async (req, res) => {
      ...
      if (!hostingConfig.isCompatible) {
        ...
        console.log(hostingConfig)
        return res.render('hosting-error', {
          headerSubNavItems: getNavItemsForStandaloneOrEmbedded(req, options),
          message: hostingConfig.messageFormattedText && prepareMessageFromApiAsHtml(hostingConfig.messageFormattedText)
        })
      }
      ...
      if (sessionValidationResponse) {
        ...
        return res.render('hosting-logged-in', {
          ...
          uploadPrototypeUrl: originalUrl.replace('npi-cloud-upload', 'hosting') + '/upload-prototype',
          ...
          remainingUploadCapacity: sessionValidationResponse.uploadCapacity - sessionValidationResponse.uploadedCount
        })
      }
      res.render('hosting-logged-out', {
        ...
        currentUrl: req.originalUrl,
        redirectUrl: originalUrl,
        loggedOutMessage: prepareMessageFromApiAsHtml(hostingConfig.loggedOutMessage)
      })
    })
    ...
    router.post('/hosting/upload-prototype', [hostingEnabled], bodyParser.urlencoded({ extended: true }), async (req, res) => {
      ...
      try {
        ...
        const tgz = await createTgz(options.prototypeDir)
        tgz.pipe(apiReq)
        ...
        if (!result.body.statusId) {
          console.error('No statusId returned from upload request')
          return res.redirect(contextPath + '/hosting?error-splash=generic')
        }
...
async function requestUploadToken (info) {
  ...
  console.log('status code issue when requesting upload token', result.body)
}
...
async function validateLatestSession (expanded = false) {
  ...
  if (sessionInfo && sessionInfo.status === 'logged-in' && sessionInfo.token) {
    try {
      ...
      console.error('problem with request (d)', e)
    }
  }
}
