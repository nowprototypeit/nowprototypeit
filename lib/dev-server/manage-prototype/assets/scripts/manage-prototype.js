        ...
        function fetchMessageAndUpdate () {
          fetch(`/manage-prototype/hosting/upload-progress-ajax?statusId=${encodeURIComponent(statusId)}`)
            ...
            .then(json => {
              ...
              } else if (json.uploadFailed) {
                const reason = json.reason ? `&reason=${encodeURIComponent(json.reason)}` : ''
                window.location.href = `/manage-prototype/hosting?error-splash=${encodeURIComponent(reason || 'generic')}`
              } else {
                console.log('no category', json)
              }
              return json.message
            })
            .then(message => {
