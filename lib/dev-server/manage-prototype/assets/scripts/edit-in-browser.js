...
async function getEditor () {
  if (!$editor) {
    ...
    $closeButton.addEventListener('click', (e) => {
      ...
      window.localStorage.removeItem('nowprototypeit-last-editor-page-url')
      focusEditor = () => {}
    })
...
async function showEditor () {
  if (!document.querySelector('.nowprototypeit-in-browser-editor')) {
    window.localStorage.setItem('nowprototypeit-last-editor-page-url', window.location.href)
...
function loadOrClearPreviousState () {
  const previousHref = window.localStorage.getItem('nowprototypeit-last-editor-page-url')
