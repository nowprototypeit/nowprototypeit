...
events.on(eventTypes.KIT_STARTED, (obj) => {
  ...
  if (!kitAlreadyStarted) {
    ...
    console.log(`http://localhost:${actualPort || port}/manage-prototype`)
    console.log('')
    console.log('The Prototype Kit is now running at:')
    console.log(`http://localhost:${actualPort || port}`)
    console.log('')
...
events.on(eventTypes.KIT_STOPPED, (info) => {
  if (info.code !== null && info.code !== 0) {
    ...
    console.log(`There's an error, you can see it above or at http://localhost:${actualPort || port}`)
    console.log('')
    events.emit(eventTypes.RELOAD_PAGE)
  }
})
