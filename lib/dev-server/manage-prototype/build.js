...
const pathToModule = path.join(process.cwd(), 'node_modules', 'nowprototypeit')
...
function getNPIDesignSystemPath () {
  const mainNodeModulesInProject = path.join(projectDir, 'node_modules')
  const mainNodeModulesInPackage = path.join(packageDir, 'node_modules')
  const pathWithinModule = path.join('@nowprototypeit', 'design-system')
  const potentialPaths = [
    path.join(mainNodeModulesInProject, 'nowprototypeit/node_modules', pathWithinModule),
    path.join(mainNodeModulesInProject, pathWithinModule),
    path.join(mainNodeModulesInPackage, 'nowprototypeit/node_modules', pathWithinModule),
    path.join(mainNodeModulesInPackage, pathWithinModule)
  ]
...
async function generateManagePrototypeCss (builtAssetsLocation) {
  ...
  await fsp.writeFile(tmpSassFile, [
    `@use "${getWindowsCompatibleRelativePath(tmpSassAndCssDir, path.join(getNPIDesignSystemPath(), 'assets', 'sass', 'styles.scss'))}" as npids with ($assets-url-for-variables: "/manage-prototype/now-prototype-it-design-system/assets");`,
    `@use "${(getWindowsCompatibleRelativePath(tmpSassAndCssDir, stylesFilePath))}";`
  ].join('\n'), 'utf8')
  try {
    const result = await sass.compileAsync(tmpSassFile, {
      ...
      loadPaths: [
        ...
        path.join(getNPIDesignSystemPath(), 'assets', 'sass')
      ],
      sourceMap: true,
