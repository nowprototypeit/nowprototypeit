...
function getEntrypointAndVersionFromNPIPackageJson () {
  return fsp.readFile(path.join(projectDir, 'node_modules', 'nowprototypeit', 'package.json'), 'utf8')
    .then(JSON.parse)
    .then(packageJson => ({
  ...
  setupCommandHandler: () => {
    ...
    events.on(eventTypes.MANAGEMENT_COMMAND_REQUEST, async (info) => {
      ...
      exec(info.command)
        ...
        .then((npiDependencyAfterCommand) => {
          ...
          if (npiDependencyBeforeCommand !== npiDependencyAfterCommand) {
            getEntrypointAndVersionFromNPIPackageJson()
              ...
              .catch(async (e) => {
                await setCommandIdsResolvedByRestart([id])
                console.log('Couldn\'t detect how to run the new kit version, stopping.', e)
        ...
        .catch((e) => {
          console.log(`Error when running command [${info.command}]`, e)
          sendUpdate(id, false, true, false)
          events.emit(eventTypes.RESUME_DEPENDENCY_WATCHING)
        })
    })
