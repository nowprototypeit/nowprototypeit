...
function sassVariables (contextPath = '', isLegacyGovukFrontend = false) {
  ...
  if (isLegacyGovukFrontend) {
    vars.push({
      ...
      value: contextPath + '/govuk-frontend/dist/govuk/assets/'
    }, {
...
async function generateManagePrototypeCss () {
  const timer = startPerformanceTimer()
  const rootDir = path.join(__dirname, 'dev-server', 'manage-prototype', 'assets', 'sass')
  const result = await sass.compileAsync(path.join(rootDir, 'manage-prototype.scss'), {
    quietDeps: true,
    loadPaths: [rootDir, path.join(__dirname, '..', 'node_modules')],
    sourceMap: true,
    sourceMapIncludeSources: true,
    style: 'expanded'
  })
  const cssDir = path.join('.tmp', 'manage-prototype-css')
  await fsp.mkdir(cssDir, { recursive: true })
  await fsp.writeFile(path.join(cssDir, 'manage-prototype.css'), result.css, 'utf8')
