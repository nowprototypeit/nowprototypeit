...
function addGovukFrontendSpecificValuesToAppLocalSync (appLocal) {
  const govukFontendPackageJsonPath = path.join(projectDir, 'node_modules', 'govuk-frontend', 'package.json')
  if (fs.existsSync(govukFontendPackageJsonPath)) {
    const entryPoint = JSON.parse(fs.readFileSync(govukFontendPackageJsonPath, 'utf8'))
    appLocal.govukFrontend = {
      assetPath: '/' + path.join(path.dirname(entryPoint.main), 'assets')
    }
  }
}
...
module.exports = {
  ...
  debounce,
  monitorEventLoop,
  addGovukFrontendSpecificValuesToAppLocalSync
}
