if (global.logTimeFromStart) {
  ...
  if (!callerLine) {
    global.logTimeFromStart('utils-loading (unknown caller)')
  } else {
...
function findAvailablePortWithUserPromiseEdition (idealPort = undefined) {
...
function findAvailablePortWithUser (callback, idealPort) {
  let port = idealPort ?? config.port
  ...
  function goThroughDir (dir = '') {
    ...
    return dirContents.map(item => {
      try {
        ...
        verboseLog('Error reading file, ignoring', e)
        return []
      }
    }).flat()
  }
...
module.exports = {
  ...
  findAvailablePortWithUserPromiseEdition,
  sleep,
  waitUntilFileExists,
