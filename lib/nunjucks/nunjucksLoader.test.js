...
describe('Nunjucks Loader', () => {
  ...
  beforeEach(() => {
    ...
    testScope.someIncludeHtml = {
      ...
      pathParts: ['app', 'views', 'includes', 'some-include.html']
    }
    testScope.someIncludeNjk = {
      ...
      pathParts: ['app', 'views', 'includes', 'some-include.njk']
    }
    testScope.fileSystem.writeFile(testScope.indexHtml.pathParts, testScope.indexHtml.fileContents)
    testScope.fileSystem.writeFile(testScope.indexNjk.pathParts, testScope.indexNjk.fileContents)
    testScope.fileSystem.writeFile(testScope.someIncludeHtml.pathParts, testScope.someIncludeHtml.fileContents)
    testScope.fileSystem.writeFile(testScope.someIncludeNjk.pathParts, testScope.someIncludeNjk.fileContents)
  ...
  it('should load an HTML file based on an absolute path', () => {
    const loader = new NunjucksLoader([path.join(testScope.viewsDir)])
  ...
  it('should fall back to njk when the html file is deleted', () => {
    ...
    testScope.fileSystem.deleteFile(testScope.someIncludeHtml.pathParts)
  ...
  it('should find the lower priority view when the higher priority view is deleted', () => {
    ...
    const primaryFilePathParts = ['primary', 'layouts', 'main.html']
    const primaryFilePath = testScope.fileSystem.writeFile(primaryFilePathParts, primaryFileContents)
    const secondaryFilePath = testScope.fileSystem.writeFile(['secondary', 'layouts', 'main.html'], secondaryFileContents)

    const loader = new NunjucksLoader([path.join(testScope.fileSystem.getRootPath(), 'primary'), path.join(testScope.fileSystem.getRootPath(), 'secondary')])
    jest.spyOn(loader, 'emit')
    ...
    testScope.fileSystem.deleteFile(primaryFilePathParts)
  ...
  describe('Production mode', () => {
    ...
    it('should not watch files in production mode', () => {
      new NunjucksLoader([path.join(testScope.viewsDir)])

      expect(testScope.chokidarOn).not.toHaveBeenCalled()
    })
  })
  describe('(temporary) mock file system', () => {
    it('should support deletion', () => {
      const pathToFile = testScope.fileSystem.writeFile(['abc.txt'], 'hello world')
      expect(fs.existsSync(pathToFile)).toBe(true)

      testScope.fileSystem.deleteFile(['abc.txt'])
