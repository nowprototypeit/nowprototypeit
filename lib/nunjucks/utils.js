...
function getPathToViewFile (name, appViews) {
  ...
  const viewExtensions = respectFileExtensions ? ['njk', 'md', 'php', 'html'] : ['njk', 'html']
  if (filename.includes('.') && fs.existsSync(name)) {
    ...
    const flat = [
      ...
      ...appViews.reduce((acc, viewDir) => acc.concat(...viewExtensions.map(ext => path.join(viewDir, name, `index.${ext}`))), [])
    ]
