...
function fork (command, options) {
  ...
  if (options.eventEmitter && options.closeEvent && forked.stderr) {
  ...
  forked.on('error', (err) => {
    if (hasSentSigterm) {
      console.log('error (no sigterm)')
    } else {
      console.log('error after sending sigterm')
    }
    console.log('Forked error event.', err)
  })
  ...
  return {
    close: async (skipForkCloseEvent = false) => {
      if (isOpen) {
        ...
        console.log('just before sigterm')
        hasSentSigterm = true
        process.kill(forked.pid, 'SIGTERM')
        console.log('just after sigterm')
      }
