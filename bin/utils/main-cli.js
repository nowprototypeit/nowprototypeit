...
global.logTimeFromStart = process.env.LOG_SERVE_PREBUILT_PERFORMANCE === 'true'
  ...
  : () => {}
const { shutdown, runShutdownFunctions } = require('../../lib/utils/shutdownHandlers')
...
async function runInit () {
  ...
  if (!argv.options['running-within-create-script'] && process.argv[3] !== '--') {
    usage()
    return await shutdown('cli-invalid-init', 2)
  }
...
async function runDev () {
  try {
    ...
    console.error('There\'s a problem running npm - this suggests that your environment isn\'t set up properly and may have problems running the kit.')
    process.exit(1)
  }
  ...
  events.on(eventTypes.FULL_KIT_RESTART, async (info) => {
    verboseLog('Picked up FULL_KIT_RESTART', info)
    if (info.message) {
      verboseLog('Displaying packaged message')
      console.log(info.message)
    } else {
      verboseLog('Displaying default message')
      console.log(`Fully restarting your prototype${info.reason ? ` ${info.reason}` : ''}.`)
    }
    verboseLog('Waiting for stop')
    await stop()
    verboseLog('Stop completed')
    // let repeats = 0
    // while (repeats++ < 10) {
    //   verboseLog('Waiting a second (%s)', repeats)
    //   await new Promise(resolve => setTimeout(resolve, 1000))
    //   verboseLog('Finished waiting a second (%s)', repeats)
    // }
    await start(info.entryPoint || kitEntryPoint)
  })

  events.on(eventTypes.FULL_KIT_STOP, async (info) => {
    verboseLog('Picked up FULL_KIT_STOP', info)
    if (info.logMessage) {
  ...
  start(kitEntryPoint)
  await scriptProcess.finishedPromise
  // console.log('script process finished - Stopping, but that might not be the right thing.')
  // await shutdown('cli')
}
...
async function runValidatePlugin () {
  await validatePlugin(getInstallLocation(), argv)
  await shutdown()
}
