...
const minimumAcceptablePercentageImprovements = {
  ...
  //
  // preBuilt: Succeeded with a 64.22% improvement
  // serve: Succeeded with a 6.23% improvement
  // dev: Succeeded with a 22.31% improvement
  //
  // They're set low to avoid this being an annoying test that fails because of natural variations between runs.
  preBuilt: 55,
  serve: 4,
  dev: 18
}
...
(async () => {
  ...
  if (controlResults.serve && actualResults.preBuilt) {
    ...
    console.log('')
    console.log("The pre-built-serve is designed for our dedicated hosting environment but we love open-source so it's available to anyone who wants to build their own custom hosted environment.")
    console.log('')
  }
  if (failureReasons.length > 0) {
    process.exit(1)
